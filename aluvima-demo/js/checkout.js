...
// El número de destino se toma automáticamente de js/config.js
// (campo: contacto.whatsapp). Si por alguna razón no se carga el config,
// se usa este valor de respaldo:
const NUMERO_DESTINO_FALLBACK = "584247247358";
// ──────────────────────────────────────────────────────────────────────────

function getNumeroDestino() {
  return (window.ALUVIMA_CONFIG?.contacto?.whatsapp) || NUMERO_DESTINO_FALLBACK;
}

function getPasarelaSeleccionada() {
  const select = document.getElementById('cart-payment-method');
  const id = select?.value;
  if (!id || !window.ALUVIMA_CONFIG?.pasarelas) return null;
  return window.ALUVIMA_CONFIG.pasarelas.find(p => p.id === id) || null;
}
...
const Checkout = (() => {
  ...
  function processCheckout() {
    ...
    // Construir URL de WhatsApp API (usa el número de config.js)
    const whatsappURL = `https://wa.me/${getNumeroDestino()}?text=${encodedMessage}`;
